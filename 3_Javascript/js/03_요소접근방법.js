...
function fnTest5(){
    ...
    for(let i =0; i<hobby.length; i++){
        ...
        // .checked 속성(redio, checkbox 전용 속성)
        // 해당 요소가 체크 되어있다면 true / 아니면 false

        if( hobby[i].checked){ // 체크가 된경우
...
function inputEnter(){
    ...
    if(window.event.key == "Enter"){ // 눌러진 키가 Enter 인 경우
...
function readValue(){ // 입력된 값을 읽어 들여 채팅창에 출력
