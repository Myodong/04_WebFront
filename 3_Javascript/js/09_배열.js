...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    const arr4 = ['사과','딸기','바나나'];  // 3칸 짜리 초기화된 배열 생성
    ...
    // 2. 배열.forEach(function(item,index){반복수행 코드}) -배열만 사용가능
    // - item  : 현재 접근중인 요소
    // - index : 현재 인덱스

    // * 여러 요소를 얻어온 경우 (HTMLCollection, NodList)는 배열이 아니므로 forEach()문을 쓸 수 없다.
    ...
    const list2 = document.querySelectorAll("#test > li");  // NodList
                                                            // HTMLCollection ,NodList : 유사 배열
